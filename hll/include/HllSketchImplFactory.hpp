...
public:
  ...
  static void deserializeAndMerge(const void *bytes, size_t len, hll_union_alloc<A> &dst, const A &allocator);
...
template<typename A>
void HllSketchImplFactory<A>::deserializeAndMerge(const void* bytes, size_t len, hll_union_alloc<A> &dst, const A& allocator) {
    // read current mode directly
    const uint8_t preInts = static_cast<const uint8_t*>(bytes)[0];
    if (preInts == hll_constants::HLL_PREINTS) {
